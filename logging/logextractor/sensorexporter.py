...
def main():
    ...
    if (args.ips):
        ...
        for ip in ips:
            ...
            ip = ip.strip()
            command = "docker run --rm --log-driver=syslog --log-opt syslog-address=tcp://100.98.26.60:5000 --log-opt syslog-facility=daemon -itd --name=%s_server -e IDRAC_IP=%s -e IDRAC_USERNAME=%s -e IDRAC_PASSWORD=%s -e ELASTIC_IP=%s -e ELASTIC_USERNAME=%s -e ELASTIC_PASSWORD=%s ajeetraina/openusm-analytics python docker_sensor_exporter.py &" % (
