...
def main():
    ...
    for ip in ips_list:
            ...
            command = "docker run --log-driver=syslog --log-opt syslog-address=tcp://0.0.0.0:5000 --log-opt syslog-facility=daemon -itd --name=%s_server -e IDRAC_IP=%s -e IDRAC_USERNAME=%s -e IDRAC_PASSWORD=%s -e ELASTIC_IP=%s -e ELASTIC_USERNAME=%s -e ELASTIC_PASSWORD=%s ajeetraina/openusm_analytics python docker_sel_exporter.py &" % (ip, ip, 'root', 'calvin', elastic_ip,elastic_username,elastic_password)
            os.system(command)
